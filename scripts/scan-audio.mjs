 ...
 *
 * After running, open src/data/tracks.ts and fill in any field marked // TODO
 */
...
function extractDate(meta) {
  // Try recording date fields in order of specificity
  const candidates = [
    meta.common?.date,
...
async function scan() {
  ...
  for (const filename of files) {
    ...
    tracks.push({
      id: slugify(title ?? basename),
      file: `/media/audio/${filename}`,
      title,
      artist,
      description,
      duration,
    ...
    if (!artist)      console.log(`      artist:     missing`);
    if (!description) console.log(`      description: missing`);
    if (!recordedAt)  console.log(`      recordedAt:  missing`);
  }

  // Build the output file
  const lines = [
    `// AUTO-GENERATED by scripts/scan-audio.mjs`,
    `// Run \`npm run scan-audio\` to regenerate from file metadata`,
    `// Fill in any field marked // TODO`,
    ``,
  ...
  for (const t of tracks) {
    ...
    lines.push(`    title: ${t.title ? JSON.stringify(t.title) : `"TODO: Add title for ${path.basename(t.file)}"`},`);
    lines.push(`    artist: ${t.artist ? JSON.stringify(t.artist) : `"TODO: Add artist"`},`);
    lines.push(`    description: ${t.description ? JSON.stringify(t.description) : `"TODO: Add description"`},`);
    lines.push(`    duration: ${JSON.stringify(t.duration)},`);
    lines.push(`    recordedAt: ${JSON.stringify(t.recordedAt)}, // from file metadata`);
    lines.push(`  },`);
  }
  ...
  console.log(`   Open the file and fill in any TODO fields.`);
}
